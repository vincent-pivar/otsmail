...
const accountService = {
	...
	async add(c, params, userId) {
		...
		// issue #433 / #426 / #494：上游遇到 is_del=1 的历史记录直接抛「该邮箱已被注销」，
		// 于是注销过的邮箱永远无法再次添加 —— 这是重复报告最多的痛点。
		// 处理方式：软删记录不再是死锁，而是先物理清理掉残留（连同它的邮件/附件），
		// 再走正常的新增流程。有 syncDelete 开关的用户本来就已经是物理删除，
		// 这里只是把「软删残留」这条历史数据路径补齐。
