...
const userService = {
	...
	/**
	 * 直接写入已算好的 hash/salt，不做长度校验、不改 KV 会话。
	 * 仅用于登录时把旧的单轮 SHA-256 哈希平滑升级为 PBKDF2。
	 */
	async updatePwd(c, userId, hash, salt) {
		await orm(c).update(user).set({ password: hash, salt: salt }).where(eq(user.userId, userId)).run();
	},
	...
	async add(c, params) {
		...
		// issue #433 / #426 / #494：管理员手动添加用户时同样不该被软删残留锁死。
		// 与 accountService.add / loginService.register 保持一致。
		if (accountRow && accountRow.isDel === isDel.DELETE) {
			await accountService.physicsDelete(c, { accountId: accountRow.accountId });
			const oldUser = await this.selectByEmailIncludeDel(c, email);
			if (oldUser && oldUser.isDel === isDel.DELETE) {
				await this.physicsDelete(c, { userIds: String(oldUser.userId) });
			}
		} else if (accountRow) {
