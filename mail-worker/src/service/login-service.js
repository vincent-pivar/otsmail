...
const loginService = {
	...
	async register(c, params, oauth = false) {
		...
		// issue #433 / #426 / #494：注销过的邮箱无法重新注册。
		// 与 accountService.add 保持一致：清掉软删残留后允许重新注册，
		// 而不是永久锁死这个地址。
		if (accountRow && accountRow.isDel === isDel.DELETE) {
			await accountService.physicsDelete(c, { accountId: accountRow.accountId });
			const oldUser = await userService.selectByEmailIncludeDel(c, email);
			if (oldUser && oldUser.isDel === isDel.DELETE) {
				await userService.physicsDelete(c, { userIds: String(oldUser.userId) });
			}
		} else if (accountRow) {
		...
		const { salt, hash } = await saltHashUtils.hashPassword(password, c);
	...
	async login(c, params, noVerifyPwd = false) {
		...
		// 口令哈希平滑升级：旧库里的单轮 SHA-256 在本次登录成功后就地换成
		// PBKDF2，用户无感。失败不影响登录（下次登录再试）。
		if (!noVerifyPwd && password && cryptoUtils.isLegacyHash(userRow.password)) {
			try {
				const { salt, hash } = await cryptoUtils.hashPassword(password, c);
				await userService.updatePwd(c, userRow.userId, hash, salt);
			} catch (e) {
				console.warn('password rehash failed (non-fatal):', e.message);
			}
		}
